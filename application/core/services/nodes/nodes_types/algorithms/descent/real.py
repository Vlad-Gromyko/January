...
class Node(INode):
    def __init__(self, special_id, config, editor, canvas, x, y, control, text, theme, **kwargs):
        ...
        values = ['Gradient', 'AdaGrad', 'RMSProp', 'Adam']
    ...
    def execute(self):
        ...
        for i in range(0, int(arguments['Число Итераций'])):
            ...
            gradient = (m_plus - m_minus) / (2 * amp) * steps
            print('m_plus   ::: ', m_plus)
            ...
            if self.combo.get() == 'Gradient':
                prognosis = velocity * gradient
            elif self.combo.get() == 'AdaGrad':
                self.ada = self.ada + gradient ** 2

                prognosis = velocity * gradient / np.sqrt(self.ada + epsilon)
            elif self.combo.get() == 'RMSProp':
                self.prop = self.prop * beta + (1 - beta) * gradient ** 2

                prognosis = velocity * gradient / np.sqrt(self.prop + epsilon)
            ...
            result = result + inertia * (u - self.previous)

            result = list(result)

            print('norm ::: ', np.linalg.norm(gradient * velocity))
