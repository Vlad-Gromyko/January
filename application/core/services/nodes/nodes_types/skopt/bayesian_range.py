...
class Node(INode):
    def __init__(self, special_id, config, editor, canvas, x, y, control, text, theme, **kwargs):
        ...
        self.add_enter_socket('MAX', self.palette['vector1d'])
        self.add_enter_socket('MIN', self.palette['vector1d'])
    ...
    def reborn_optimizer(self):
        ...
        space = [Real(arguments['MIN'][i], arguments['MAX'][i], name=f'x{i}') for i in range(len(arguments['Решение']))]

        estimator = self.combobox.get()
        acq = self.combobox_acq.get()
        self.optimizer = Optimizer(space, base_estimator=estimator, acq_func=acq)
    ...
    @staticmethod
    def create_info():
        return Node, 'Байесовская оптимизация Range', 'strategy'
