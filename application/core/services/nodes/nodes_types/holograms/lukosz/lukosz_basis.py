...
from application.core.utility.fast_lukosz import lukosz_by_number
import numpy as np
...
class Node(INode):
    ...
    def execute(self):
        ...
        for i in range(int(start), int(start+length)):
            if str(i) not in self.processed.keys():
                self.processed[str(i)] = lukosz_by_number(i, self.rho, self.phi)
    ...
    @staticmethod
    def create_info():
        return Node, 'Лукош-Браат', 'Базисы'
